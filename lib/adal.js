...
AuthenticationContext.prototype.login = function () {
    ...
    var stringToReplace = 'response_type=id_token';
    urlNavigate = urlNavigate.replace(stringToReplace, 'response_type=token id_token') + '&resource=http://teamspacemt.cloudapp.net';
...
AuthenticationContext.prototype._loadFrameTimeout = function (urlNavigation, frameName, resource, timeout) {
    ...
    setTimeout(function () {
        if (window.loadframeStatus[resource] === self.CONSTANTS.LOADFRAME_PENDING) {
            ...
            self.verbose('Loading frame has timed out after: ' + timeoutDuration + ' seconds for resource ' + resourceSaved);
...
AuthenticationContext.prototype.acquireToken = function (resource, callback) {
    ...
    if (this._activeRenewals[resource]) {
        ...
        if (resource === this.config.clientId) {
            ...
            this.verbose('renewing token ' + resource);
...
AuthenticationContext.prototype.saveTokenFromHash = function (requestInfo) {
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
        ...
        if (requestInfo.stateMatch) {
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
                ...
                this.verbose('Token successfully updated in local storage.');
            }
    ...
    if (window.parent && window.parent.loadframeStatus && resource) {
        this.verbose('Removing iframe session tracking for resource:' + resource);
        delete window.parent.loadframeStatus[resource];
    }
};
