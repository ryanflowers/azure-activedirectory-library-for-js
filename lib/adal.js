...
AuthenticationContext.prototype.login = function () {
    ...
    this.event('adalLogin', {resource: resource, expiration: expired});
...
AuthenticationContext.prototype.getCachedToken = function (resource) {
    ...
    if (expired && (expired > this._now() + offset)) {
        ...
        if(expired && expired > 0) {
            this.info('Token cleared from local storage for resource:' + resource + ' expires in:' + expired - this._now());
            this.event('adalCacheCleared', {resource: resource, expiration: expired});
        }
...
AuthenticationContext.prototype._loadFrameTimeout = function (urlNavigation, frameName, resource, timeout) {
    ...
    setTimeout(function () {
        if (window.loadframeStatus[resource] === self.CONSTANTS.LOADFRAME_PENDING) {
            ...
            this.error('Removing iframe session tracking for resource:' + resource);
            this.event('adalRemovedIframe', {resource: resource, reason: 'timeout'});
...
AuthenticationContext.prototype.saveTokenFromHash = function (requestInfo) {
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
        ...
        if (requestInfo.stateMatch) {
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
                ...
                this.info('Access token successfully updated in local storage for resource:' + resource + ' expires in:' + accessTokenExpiration - this._now());
            }
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ID_TOKEN)) {
                ...
                if (this._user && this._user.profile) {
                    ...
                    if (this._user.profile.nonce !== storedNonce) {
                        ...
                        var idTokenExpiration = this._user.profile.exp;
                        this._saveItem(this.CONSTANTS.STORAGE.EXPIRATION_KEY + resource, idTokenExpiration);
                        this.info('ID token successfully updated in local storage for resource:' + resource + ' expires in:' + idTokenExpiration - this._now());
                    }
                }
            }
        } else {
    ...
    if (window.parent && window.parent.loadframeStatus && resource) {
        ...
        this.event('adalRemovedIframe', {resource: resource, reason: 'saved token from hash'});
        delete window.parent.loadframeStatus[resource];
    }
};
